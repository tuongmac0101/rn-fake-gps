...
export interface IHttpClientConfig extends CreateAxiosDefaults {
  beforeRequest: <TDataRequest = any>(
    ...
    | InternalAxiosRequestConfig<TDataRequest>
    | Promise<InternalAxiosRequestConfig<TDataRequest>>;
...
export interface IRequestConfigParams<TDataRequest = any>
  extends IRequestConfigBase<TDataRequest> {
  params?: TDataRequest;
}

export interface IRequestConfigBody<TDataRequest = any>
  extends IRequestConfigBase<TDataRequest> {
  data?: TDataRequest;
}

export interface IRequestConfig<TDataRequest = any>
  extends IRequestConfigBase<TDataRequest> {
  data?: TDataRequest;
  params?: TDataRequest;
}
...
const HttpClient = (baseConfig: IHttpClientConfig) => {
  const { beforeRequest, handleError, handleResponse, ...configInit } =
    baseConfig;
  const initApi = () => {
  ...
  const postForm = <TDataResponse = any, TDataRequest = any>(
    ...
    return executeApi<TDataResponse>(
      instanceApi.postForm(endPoint, body, config)
    );
  };
  const putForm = <TDataResponse = any, TDataRequest = any>(
    ...
    return executeApi<TDataResponse>(
      instanceApi.putForm(endPoint, body, config)
    );
  };
  const patchForm = <TDataResponse = any, TDataRequest = any>(
    ...
    return executeApi<TDataResponse>(
      instanceApi.patchForm(endPoint, body, config)
    );
  };
...
export const createHttpClient = (baseConfig: IHttpClientConfig) =>
